...
internal class TrieBytes : ITrie
{
    ...
    // Functions that takes an element and adds it to the trie.
    public bool AddWithKey(string element, char key)
    {
        if (Contains(element))
        {
            return false;
        ...
        {
            return AddWithKeyRecursion(root, element, key);
        }
    }

    bool AddWithKeyRecursion(TrieNode node, string remainingString, char key)
    {
        ...
        if (subString.Length == 0)
        {
            ...
            Size++;
            node.SubNodes[prefix].Key = key;
            return true;
        }
        else
        {
            return AddWithKeyRecursion(node.SubNodes[prefix], subString, key);
        }
    }
    ...
    public char GetKey(string element)
    {
        TrieNode tempNode = root;
        foreach (char character in element)
        {
    ...
    {
        TrieNode tempNode = root.SubNodes[key];
